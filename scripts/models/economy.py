...
class EcoProfile:
    ...
    def load(user):
        ...
        timeAware_Collection = mongoClient.DBot.economy.with_options(
        ...
        if profileDoc is None:
            user = user
            balance = EcoProfile.STARTING_MONEY
            timeCreation = utcNow()
    ...
    def changeBalance(self, changeAmount, forced=False):
        if forced or self.checkBalance(changeAmount):
            ...
            dbClient.getClient().DBot.economy.update_one(
                {"user.id": self.user.id}, {"$set": {"balance": self.balance}})
            self.unlock()
            return 0
        else:
            return -1
    ...
    def collect(self):
        if self._ableToCollect():
            self.changeBalance(EcoProfile.COLLECTION_MONEY)
            dbClient.getClient().DBot.economy.update_one(
                {"user.id": self.user.id}, {"$set": {"timeCollection": utcNow()}})

            timeAware_Collection = dbClient.getClient().DBot.economy.with_options(
                codec_options=CodecOptions(tz_aware=True, tzinfo=TIMEZONE))
            self.timeCollection = timeAware_Collection.find_one(
                {"user.id": self.user.id})["timeCollection"]
            return 0
        else:
            return -1
    ...
    def lock(self):
        self._lock = True
        dbClient.getClient().DBot.economy.update_one(
            {"user.id": self.user.id}, {"$set": {"lock": self._lock}})
    ...
    def _save(self):
        ...
        if mongoClient.DBot.economy.count_documents({"user.id": self.user.id}) == 0:
            ...
            mongoClient.DBot.economy.replace_one(
                {"user.id": self.user.id}, self._makeDoc())
