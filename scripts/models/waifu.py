...
from scripts.helpers.aux_f import TIMEZONE, utcNow, dateNow, timeToDate
...
class WaifuProfile:
    ...
    def load(user):
        ...
        timeAware_Collection = mongoClient.DBot.waifu.with_options(
        ...
        profile = WaifuProfile(
            user, waifuList, waifuFavorite, timeCreation, timeSummoning)
        if profileDoc is None:
            profile._save()
    ...
    def addWaifu(self, waifu):
        ...
        dbClient.getClient().DBot.waifu.update_one(
            {"user.id": self.user.id}, {"$set": {"waifuList": self.waifuList}})
    ...
    def getTotalValue(self):
        waifus = list(dbClient.getClient().DBot.waifus.find(
            {"MAL_data.charID": {"$in": self.waifuList}}))
    ...
    def setFavorite(self, waifuID):
        if not (waifuID in self.waifuList):
            ...
            dbClient.getClient().DBot.waifu.update_one({"user.id": self.user.id}, {
                "$set": {"waifuFavorite": self.waifuFavorite}})
            return 0
    ...
    def getDuplicateWaifuIDs(self):
        ...
        duplicateDict = {waifuID: countDict[waifuID]
                         for waifuID in countDict.keys() if countDict[waifuID] > 1}
        return duplicateDict

    def summonWaifu(self):
        if self._ableToSummon():
            dbClient.getClient().DBot.waifu.update_one(
                {"user.id": self.user.id}, {"$set": {"timeSummoning": utcNow()}})
            timeAware_Collection = dbClient.getClient().DBot.waifu.with_options(
                codec_options=CodecOptions(tz_aware=True, tzinfo=TIMEZONE))
            self.timeSummoning = timeAware_Collection.find_one(
                {"user.id": self.user.id})["timeSummoning"]
            return 0
        else:
            return -1
    ...
    def _save(self):
        ...
        if mongoClient.DBot.waifu.count_documents({"user.id": self.user.id}) == 0:
            ...
            mongoClient.DBot.waifu.reaplace_one(
                {"user.id": self.user.id}, profileDoc)
